...
export function ThemeToggle({ collapsed, variant = "sidebar" }: ThemeToggleProps) {
  ...
  if (variant === "header") {
    return (
      <div>
        ...
        />
        <label
          className="flex items-center justify-center cursor-pointer w-8 h-8 hover:bg-gray-100 dark:hover:bg-gray-700/50 rounded-full transition-colors"
          htmlFor="light-switch"
        >
          {/* Sun icon (shown in light mode) */}
          <svg
            className="dark:hidden fill-current text-gray-500/80"
            width={16}
            height={16}
            viewBox="0 0 16 16"
          >
          ...
          <svg
            className="hidden dark:block fill-current text-gray-400/80"
            width={16}
            height={16}
            viewBox="0 0 16 16"
          >
  ...
  return (
    <div className="relative group">
      ...
      >
        {/* Sun/Moon inline SVG for sidebar variant too */}
