    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-card rounded-xl border border-border p-5">
          ...
          <p className="text-xs text-muted-foreground mt-1">na rede no período</p>
        </div>
        ...
        <div className="bg-card rounded-xl border border-border p-5">
          <div className="flex items-center justify-between mb-3">
            <span className="text-sm font-medium text-muted-foreground">Perda Total</span>
          ...
          <p className="text-xs text-muted-foreground mt-1">{data ? `${data.kpis.perdaPct}% da rede` : ""}</p>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <div className="bg-card rounded-xl border border-border p-5">
          <h3 className="text-sm font-semibold text-foreground mb-4">Top 7 Subestações — Injetado vs Faturado</h3>
          {loading ? (
        ...
        {/* RadarChart by ilha */}
        <div className="bg-card rounded-xl border border-border p-5">
          <h3 className="text-sm font-semibold text-foreground mb-4">Índice de Risco por Ilha (%)</h3>
          {loading ? (
            <Skeleton className="h-64 rounded-lg" />
      ...
      <div className="bg-card rounded-xl border border-border">
        <div className="px-5 py-4 border-b border-border">
          <h3 className="text-sm font-semibold text-foreground">Detalhe por Subestação</h3>
        </div>
        {loading ? (
                ...
                {(data?.tabela ?? []).length === 0 ? (
                  ...
                  data?.tabela.map((r) => (
                    <tr key={r.id} className="hover:bg-muted/30 transition-colors">
                      ...
                      <td className="px-5 py-3 text-right text-foreground tabular-nums">{r.kwh_injetado.toLocaleString("pt-CV")}</td>
                      <td className="px-5 py-3 text-right text-foreground tabular-nums">{r.kwh_faturado.toLocaleString("pt-CV")}</td>
                      ...
                      <td className="px-5 py-3 text-right text-foreground tabular-nums">{formatCVE(r.cve_estimado)}</td>
                    </tr>
                  ))
                )}
              </tbody>
