/**
 * Constantes partilhadas do Motor de Scoring — MIRROR Deno
 *
 * Mirror exato de `src/modules/scoring/constants.ts` para uso em Edge Functions
 * (Deno). Mantido em sincronização pelo teste:
 *   src/__tests__/scoring-parity.test.ts
 *
 * Qualquer alteração aqui DEVE ser replicada no canónico (`src/modules/scoring/constants.ts`).
 * O teste de paridade falha se os valores divergirem.
 */
...
export const R8_LOOKBACK_MAX = 24;
